...
export const LikedPost: React.FC<PropsType> = ({
	likedPosts,
	removeFromSaved,
}) => {
	...
	return (
		<div className='col-lg-12'>
			{likedPosts.map((p, i) => {
				return (
					<div className='user-posts__item  border rounded' id={p.id} key={i}>
						<div className='user-posts__item-head mb-3'>
							<img
								className='user-posts__item-img'
								src={p.owner.picture}
								alt={photo}
							/>
							...
							<span className='user-posts__item-date'>{p.publishDate}</span>
						</div>
						<div className='user-posts__item-content'>
							...
							<div className='user-post__item-content-info'>
								...
								<div className='user-posts__btn-section'>
									...
									<button
										className='user-posts__item-btn'
										onClick={() => onLikeChange(p.id)}>
