...
let initialState = {
	...
	postMessages: [
		...
		{
			id: 2,
			message: 'HTML5/CSS3, JavaScript, TypeScript, React/Redux, REST-API',
		},
	],
...
const profileReducer = (
	...
	switch (action.type) {
		...
		case 'profile/DELETE_POST':
			return {
				...state,
				myPosts: state.myPosts.filter((p: any) => p.id !== action.id),
			}
...
export const actions = {
	...
	setProfileStatus: (status: string) =>
		({ type: 'profile/SET_PROFILE_STATUS', status } as const),
